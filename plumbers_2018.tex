...
We present various optimization that fall into two broad categories:
...
with what
user-space driver techniques, such as DPDK, can deliver. With these
optimizations, we more than double the throughput performance: from 15
Mpps to 39 Mpps for Rx and from 25 Mpps to 68 Mpps for Tx. We have
limited the optimization proposals to patch sets that we believe are
acceptable to the networking community and non-intrusive to other
component with the exception of XDP~\cite{xdp} in some cases, as
AF\_XDP uses XDP for its Rx data path.
...
\item Introduce a new {\tt bind} option in which the user does not have to
  supply an XDP program and all traffic on the specified queue id is
  sent up to the socket. This is realized in AF\_XDP by loading a
  built-in XDP program and this program and the path leading up to it
  can be optimized leading to substantial performance
...
\item Introduce an execution context in the XDP code instead of using
  ...
  importantly, it becomes possible to implement a number of
  other performance improvements based on having information more
  readily available.
...
results in section \ref{sec:exp:res}. The paper is ended
(sections \ref{sec:future}, \ref{sec:conclutions}
and \ref{sec:thanks}) with some discussions and future work and
...
\begin{figure}[ht]
\resizebox{.5\textwidth}{!}{\input{xdp.pdf_t}}
...
seen in Figure~\ref{fig:desc_structs}, the descriptor format of these
...
ret = setsockopt(sfd, SOL_XDP, XDP_UMEM_FILL_RING, &size, sizeof(size));
size = nr_descs_in_completion_queue;
ret = setsockopt(sfd, SOL_XDP, XDP_UMEM_COMPLETION_RING, &size, sizeof(size));

size = nr_descs_in_rx_queue;
ret = setsockopt(sfd, SOL_XDP, XDP_RX_RING, &size, sizeof(size));
size = nr_descs_in_tx_queue;
ret = setsockopt(sfd, SOL_XDP, XDP_TX_RING, &size, sizeof(size));

/* Get the structure of the queues */
getsockopt(sfd, SOL_XDP, XDP_MMAP_OFFSETS, &off, &optlen);

fill_q = mmap(NULL, off.fr.desc * nr_descs_in_fill_queue * sizeof(u64),
...
Figure~\ref{fig:ctrl_path} shows the control path in
...
\label{fig:rx_data_path}
\end{figure}

The data path is simpler and is shown in
Figure~\ref{fig:rx_data_path}. It comes in two main flavors: either a
...
\label{fig:tx_data_path}
\end{figure}

For the Tx path shown in Figure~\ref{fig:tx_data_path}, a descriptor
...
\begin{figure}[ht]
\resizebox{.5\textwidth}{!}{\input{two_vs_one_core.pdf_t}}
...
in the net/xdp directory and the headers in
\texttt{include/net/xdp\_sock.h}. Note that some of the Rx path is in
the XDP path and that code is in \texttt{net/core/filter.c}. An
example sample program can be found in \texttt{samples/bpf/} {\tt
  xdpsock\_user.c} and the XDP program used to route the packets are
in \texttt{samples/bpf/xdpsock\_kern.c}.
...
\begin{figure}[t]
\resizebox{0.5\textwidth}{!}{\input{in_order.pdf_t}}
...
indicate which page buffers should be returned by indicating what chunk
it belongs to. In the current implementation the chunk size can be 2K
or 4K, but with huge pages this could be for example 64K. So
...
handler that only deals with XDP Rx and/or Tx rings and provide a
slim and highly optimized path from there. Another example would be to
go to 16-byte descriptors for those queues as these can be handled
faster by the I40E NIC and also consumes less memory which could lead
...
experimented with this and it provided better throughput as it used the
instruction cache more efficiently. But we have not had the time to
implement this in the latest Linux kernel with AF\_XDP. Jesper
...
by between 20\% and 25\%, it also cuts down the number of used cores to
one. Measured on a per core basis, busy poll() actually increases
...
levels, Tx and an application that actually touches the data,
l2fwd, offers comparable performance to DPDK when we run in
...
Duyck, Mykyta Iziumtsev, Jakub Kicinski, Song Liu, Sridhar Samudrala,
Yonghong Song, Alexei Starovoitov, William Tu, Anil Vasudevan, Jingjng
Wu, and Qi Zhang. We owe you one.
